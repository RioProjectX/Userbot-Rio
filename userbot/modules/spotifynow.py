    ...
    try:
        async with event.client.conversation(chat) as conv:
            ...
            if response.text.startswith("You're"):
                await event.edit(
                    "**Anda tidak mendengarkan apa pun di Spotify saat ini**"
                )
                await event.client.delete_messages(conv.chat_id, [msg.id, response.id])
                return
            if response.text.startswith("Ads."):
                await event.edit("**Anda sedang mendengarkan iklan yang mengganggu.**")
                await event.client.delete_messages(conv.chat_id, [msg.id, response.id])
                return
            else:
                if input_str == "s":
        ...
        return await event.edit("**Error:** @SpotifyNowBot **tidak merespon!**")
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `.spotnow`\
        \n  •  **Function : **Menampilkan apa yang Anda dengarkan di spotify via @SpotifyNowBot\
        \n\n  •  **Syntax :** `.spotnow s`\
        \n  •  **Function : **Menampilkan apa yang Anda dengarkan di spotify via @SpotifyNowBot tapi kirim sebagai stiker\
    "
    }
)
